...
public class IfYouGiveASeedAFertilizerPart2 {
    ...
    public static long getOutput(List<String> lines) {
        ...
        String[] firstLine = lines.remove(0).trim().split(": ")[1].split(" ");
        ...
        for(int k = 0; k < 10; k++) {
            for(int i = 0; i < firstLine.length; i+=2) {
                long start = Long.parseLong(firstLine[i]);
                long count = Long.parseLong(firstLine[i + 1]);

                for(long j = start + k; j < start + count; j+=10) {
                    seeds.add(j);
                }
            }
