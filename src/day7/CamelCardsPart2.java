...
public class CamelCardsPart2 {
    ...
    private static class Hand implements Comparable<Hand> {
        ...
        private int calculateStrength() {
            int strength = HandStrengthEnum.HIGH_CARD.getIntValue();

            List<String> copyOfCards = new ArrayList<>(List.copyOf(cards));
            Map<String, Long> counts = copyOfCards.stream().collect(Collectors.groupingBy(e -> e, Collectors.counting()));
            String mostCommonCard = Collections.max(counts.entrySet(), Map.Entry.comparingByValue()).getKey();

            if(mostCommonCard.equals("J")) {
                Long max = Long.MIN_VALUE;
                String maxKey = "J";
                for(String key : counts.keySet()) {
                    if(!key.equals("J") && max <= counts.get(key)) {
                        max = counts.get(key);
                        maxKey = key;
                    }
                }

                mostCommonCard = maxKey;
            }

            Collections.replaceAll(copyOfCards, "J", mostCommonCard);

            if(isFiveOfAKind(copyOfCards)) {
                return HandStrengthEnum.FIVE_OF_A_KIND.getIntValue();
            }

            if(isFourOfAKind(copyOfCards)) {
                return HandStrengthEnum.FOUR_OF_A_KIND.getIntValue();
            }

            if(isAFullHouse(copyOfCards)) {
                return HandStrengthEnum.FULL_HOUSE.getIntValue();
            }

            if(isThreeOfAKind(copyOfCards)) {
                return HandStrengthEnum.THREE_OF_A_KIND.getIntValue();
            }

            if(isTwoPair(copyOfCards)) {
                return HandStrengthEnum.TWO_PAIR.getIntValue();
            }

            if(isOnePair(copyOfCards)) {
